module PFM
  ...
  class Daycare
    ...
    def try_to_lay(daycare)
      ...
      log_debug "==== Pension Infos ====\nLay attempt : #{daycare[:egg] ? 'Success' : 'Failure'}\n"
    end
    ...
    def inherit(pokemon, parents)
      ...
      male, female = assign_gender(*parents)
      # Inherit sequence
      pokemon.captured_with = female.captured_with unless NON_INHERITED_BALL.include?(data_item(female.captured_with).db_symbol)

...
1. Download the source code from [this link](https://www.sfml-dev.org/download/sfml/2.5.1/index.php)
...
11. Open the file `src/VideoStream.cpp` and replace the missing constants to `AV_<constant_name>` (line 63 & 280)
12. Copy the file `bin/libsfeMovie.dll.a` to `<RUBY>/msys32/mingw32/lib/`
13. Copy the file `bin/libsfeMovie.dll` to `<RUBY>/msys32/mingw32/bin/`
14. Copy the file `bin/libsfeMovie.dll` to `<PROJECT>/ruby_builtin_dlls/`
15. Copy the folder `include/sfeMovie` to `<RUBY>/msys32/mingw32/include/`

...
```ruby
heuristic = 0.75 + 0.125 * (1 + Math.erf((power * effectiveness - POWER_MEAN) / POWER_STD))
```

Where `power` is the move's real_base_power, `effectiveness` is the result of the `move_effectiveness` method call, and MEAN and STANDARD_DEVIATION are arbitrary chosen constants to be respectively 200 and 150, in order to draw a curve assuming the majority of moves will have an effective power between 50 and 350.

This formula yields values between 0.75 & 1.0 where 0.75 is a move with 0 effectiveness, and 1.0 is the most powerful move (effective power > 700).

This formula might be inaccurate for certain use cases. Feel free to report any issue regarding the choice of moves based on their sole effective power (without external modifiers whatsoever).

Afterwards, a special modifier is computed to the heuristic through the method `Battle::AI::MoveHeuristicBase.compute()`. This modifier is set by default to `Math.sqrt(user.atk / target.dfe)` (depending on the category). Some moves might implement their own factor, for example protect might get a decreasing factor depending on the number of time it was used.
